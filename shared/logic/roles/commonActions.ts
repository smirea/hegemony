...
export function createProposeBill(role: WorkingClassRole | MiddleClassRole | CapitalistRole | StateRole) {
	return {
		proposeBill: createAction({
			...
			condition: () => [
				[
					'hasVotes',
					Object.values(role.game.data.board.policyProposals).filter(p => p && p.role === role.id).length < 3,
				],
			],
			validateInput: ({ policy, value }) => [
				...
				['isAdjacent', Math.abs(role.game.data.board.policies[policy] - value) === 1],
			],
			run: ({ policy, value }) => {
...
export function createPass(_role: WorkingClassRole | MiddleClassRole | CapitalistRole | StateRole) {
	return {
		pass: createAction({
			run: () => void 0,
		}),
...
export function createPayLoan(role: WorkingClassRole | MiddleClassRole | CapitalistRole | StateRole) {
	return {
		payLoan: createAction({
			...
			run: () => {
				...
				role.data.resources.money.remove(50, { useCapital: role.id === RoleEnum.capitalist } as any);
			},
		}),
	};
}
...
export function createUseHealthcare(role: WorkingClassRole | MiddleClassRole) {
	return {
		useHealthcare: createAction({
			condition: () => [['hasHealthcare', role.data.resources.healthcare.value >= role.getPopulation()]],
			run: () => {
...
export function createUseEducation(role: WorkingClassRole | MiddleClassRole) {
	return {
		useEducation: createAction({
			...
			condition: () => [['hasEducation', role.data.resources.education.value >= role.getPopulation()]],
			validateInput: ({ type }) => [['hasAvailableWorkers', role.data.availableWorkers[type] > 0]],
			run: ({ workerId, type }) => {
...
export function createUseLuxury(role: WorkingClassRole | MiddleClassRole) {
	return {
		useLuxury: createAction({
			condition: () => [['hasLuxury', role.data.resources.luxury.value >= role.getPopulation()]],
			run: () => {
...
export function createAdjustWages(role: MiddleClassRole | CapitalistRole | StateRole) {
	return {
		adjustWages: createAction({
			...
			validateInput: toAdjust => toAdjust.map(({ wages }) => ['minWage', role.game.isWageAllowed(wages)]),
			run: toAdjust => {
				for (const { companyId, wages } of toAdjust) {
					const company = role.company(companyId);
					const isHigher = Number(wages.slice(1)) > Number(company.wages.slice(1));
					company.wages = wages;
					if (isHigher) {
...
export function createSwapWorkers(role: WorkingClassRole | MiddleClassRole) {
	return {
		swapWorkers: createAction({
			...
			condition: () => [['hasUnemployedWorkers', role.data.workers.filter(w => w.company == null).length > 0]],
			run: tuples => {
...
/** add 3 cubes to bag */
export function createApplyPoliticalPressure(role: WorkingClassRole | MiddleClassRole | CapitalistRole) {
	return {
...
export function createAssignWorkers(role: WorkingClassRole | MiddleClassRole) {
	return {
		assignWorkers: createAction({
			...
			validateInput: toAssign => role.game.validateAssignWorkers(toAssign, role.id),
			run: toAssign => {
...
export function createBuyGoodsAndServices(role: WorkingClassRole | MiddleClassRole) {
	return {
		buyGoodsAndServices: createAction({
			...
			validateInput: toBuy => [
				['oneResource', new Set(toBuy.map(item => item.resource)).size <= 1],
				['maxTwoSources', new Set(toBuy.map(item => item.source)).size <= 2],
				['sourceLimit', toBuy.every(item => item.count <= role.getPopulation())],
			],
			run(toBuy) {
				for (const { resource, count, source } of toBuy) {
					if (source === 'foreign-market') {
						if (resource !== ResourceEnumSchema.enum.food && resource !== ResourceEnumSchema.enum.luxury)
...
export function createBuildCompany(role: MiddleClassRole | CapitalistRole) {
	return {
		buildCompany: createAction({
			...
			condition: () => [
				['hasSpace', role.data.companies.length < (role.id === RoleEnum.capitalist ? 12 : 8)],
				['hasMarket', role.data.companyMarket.length > 0],
			],
			validateInput: ({ companyId, workers }) => [
				['inMarket', role.data.companyMarket.includes(companyId)],
				['workersFit', workers.length === 0 || role.game.canWorkersFillCompany(workers, companyId)],
			],
			run: ({ companyId, workers }) => {
				role.data.resources.money.remove(role.game.getCompanyDefinition(companyId).cost);
				role.data.companyMarket = role.data.companyMarket.filter(id => id !== companyId);
	...
	return {
		sellCompany: createAction({
			...
			validateInput: companyId => [
				[
					'noCommittedWorkers',
					role.company(companyId).workers.every(workerId => !role.game.getWorkerById(workerId).worker.committed),
				],
			],
			run(companyId) {
				const company = role.company(companyId);
...
export function createSellToForeignMarket(role: MiddleClassRole | CapitalistRole | StateRole) {
	return {
		sellToForeignMarket: createAction({
			playerInputSchema: z.record(TradeableResourceSchema, z.tuple([z.boolean(), z.boolean()])),
			run(toSell) {
