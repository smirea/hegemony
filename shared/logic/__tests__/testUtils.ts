...
export function createGameUtils() {
	...
	const findAction = (type: ActionEventName, n = 0) => game.data.actionQueue.filter(a => a.type === type).at(n)!;
	...
	const assignWorkers = (ids: CompanyWorker['id'][], companyId: CompanyCard['id'], diff?: Partial<CompanyWorker>) => {
	...
	const initGame = async (
		...
		game.data.board.decks.foreignMarketDeck = new Deck(
			'test:foreign-market',
			[
				makeForeignMarketCard('f1', [1, 1, 5, 10], [1, 1, 5, 10], [1, 1, 5, 10], [1, 1, 5, 10]),
		...
		if (companyDecks === 'empty') {
			game.data.roles.middleClass.data.companyDeck = new Deck('test:middleClass companies', []);
