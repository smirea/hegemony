...
export default class Game {
	...
	setupRound() {
		...
		++this.data.round;
		this.data.turn = 0;
		const { foreignMarketDeck: foreignMarketCards, businessDealDeck: businessDealCards } = this.data.board.decks;
		this.data.board.foreignMarketCard = foreignMarketCards.draw().id;
		if (this.ifPolicy('6A')) {
			this.data.board.businessDealCards = [businessDealCards.draw().id, businessDealCards.draw().id];
	...
	readonly actions = {
		...
		roleNext: createAction({
			run: () => {
				...
				if (this.data.currentRoleName == this.data.players[this.data.players.length - 1].role) {
				...
				if (this.data.currentRoleName == null) {
					...
					this.data.currentRoleName =
						this.data.players[this.data.players.findIndex(p => p.role === this.data.currentRoleName) + 1].role;
				}
		...
		roleCurrent: createAction({
			run: () => {
				if (this.data.roles[this.data.currentRoleName!].data.usedActions.length >= 2) return this.next('game:roleNext');
