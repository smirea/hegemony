...
type Benefit = { type: 'resource'; resource: Resource; amount: number } | { type: 'voting-cube'; amount: number };
...
export default class StateRole extends AbstractRole<typeof RoleEnum.state, StateData> {
	...
	onBuyGoods(roleName: RoleNameNoState, resource: CompanyTradeableResource, count: number) {
		if (resource !== ResourceEnumSchema.enum.education && resource !== ResourceEnumSchema.enum.healthcare) return;
