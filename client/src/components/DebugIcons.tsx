...
const icons = Object.entries(import.meta.glob<{ default: React.FC<any> }>('./icons/*.tsx', { eager: true })).sort(
	([fileNameA], [fileNameB]) => fileNameA.localeCompare(fileNameB),
);

export default function DebugIcons() {
	const Target =
		location.hash && icons.find(i => i[0].toLowerCase().includes(location.hash.slice(1).toLowerCase()))?.[1].default;
	return (
		<Root>
			<h3>DebugIcons</h3>
			{Target && (
