...
- Company cards with rulebook setup meaning include a `setup` object for starting status, player-count public-company filtering, public-sector row, and setup wage. A bullet next to the printed cost identifies the starting Capitalist/Middle Class company copy when a name has duplicates.
...
- Class action card ids are normalized from the visible title plus copy index, and `content` starts at the visible title while `rawText` preserves the noisy OCR. Base action decks should use `stateEffectsCoverage: "complete"` with declarative `stateEffects`; validation rejects partial or unparsed base action cards.
- Event cards now use normalized visible titles, tasks, no-action penalties, and choice/reward text. `stateEffectsCoverage: "partial"` means the card has enough text structure for implementation but conditional reward scaling may still need a dedicated effect primitive.
...
- `immigration-cards/deck.ts` records exactly one Working Class worker and one Middle Class worker per physical card, as shown on the image. The 10/15 specialized-worker distribution is validated separately and should not be represented as per-card quantities.
