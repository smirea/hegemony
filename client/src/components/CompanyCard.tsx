...
const Production: React.FC<{ value: number; small?: boolean; industry: Industry }> = ({ value, small, industry }) => (
...
const ExtraProduction: React.FC<{
	...
	<div className='row' data-spacing='.125' data-align='end' style={{ fontSize: '1rem', opacity: active ? 1 : 0.5 }}>
		<div>(</div>
		{icon}
		<div>+{value}</div>
		<div>)</div>
			...
			<Workers {...props}>
				{companyDef.workers.map((workerDef, i) => {
					if (!company.workers[i]) {
						...
						const content = (
							<WorkerIcon key={i} status='empty' role={workerDef.roles[0]} type={workerDef.type} height={iconHeight} />
						);
