...
function createActionEventMap() {
	...
	for (const { type, typeValue } of todo) {
		result.push(splitIfNeeded([`'${type}': ActionEventFromAction<`, `'${type}',`, ' ' + typeValue, '>;']));
	}
	return result;
}
...
function createPlayerInputDataMap() {
	...
	for (const { type, action, typeValue } of todo) {
		...
		result.push(splitIfNeeded([`'${type}': z.infer<`, `NonNullable<${typeValue}['playerInputSchema']>`, '>;']));
	}
	return result;
}
...
const splitIfNeeded = (chunks: string[]) => {
	...
	return [tab1 + chunks[0], ...chunks.slice(1, -1).map(c => tab2 + c.trim()), tab1 + chunks[chunks.length - 1]].join(
		'\n',
	);
};
