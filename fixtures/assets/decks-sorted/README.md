...
- Class action card ids are normalized from the visible title plus copy index, and `content` starts at the visible title while `rawText` preserves the noisy OCR. Base action decks should not use `stateEffectsCoverage: "unparsed"`; `"partial"` means declarative effects exist but choices/conditions may still need refinement.
