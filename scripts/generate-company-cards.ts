...
function init() {
	...
	for (const [name, industry, production, extra, l3, l2, l1, workers, educated, cost] of companies.capitalist) {
		...
		if (workers > 0) {
			workersStr = [
				...
				...Array.from(
					{ length: workers },
					(_, i) => `worker('both', '${educated && i === 0 ? industry : 'unskilled'}'),`,
				).map(x => '        ' + x),
				'    ],',
			];
		}
	...
	for (const [name, industry, production, extra, l3, l2, l1, workers, educated, cost] of companies.middle) {
		let workersStr: string[] = ['    workers: []'];
		if (workers > 0) {
			workersStr = [
				'    workers: [',
				`        worker('m', '${industry}'),`,
				'        ' + (extra ? `worker('w', '${educated ? industry : 'unskilled'}', true)` : "worker('m', 'unskilled')"),
				'    ],',
			];
		}
		console.log(
	...
	for (const [name, industry, production, _extra, l3, l2, l1, workers, _educated, cost] of companies.state) {
		console.log(
			[
				'{',
