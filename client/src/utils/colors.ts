...
export function defineColors() {
	...
	styleTag.innerHTML = [
		...
		'',
		':root {',
		...expanded.definitions.map(({ varDefine, paramDefine }) => ['    ' + varDefine, '    ' + paramDefine]).flat(),
		'}',
	].join('\n');
...
function expand() {
	...
	type ConvertColor<T extends RGBA | Record<string, RGBA>, O> = T extends RGBA ? O : { [k in keyof T]: O };
