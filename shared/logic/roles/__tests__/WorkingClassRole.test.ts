...
import { createGameUtils, createTestCompanyCard, createTestWorker } from '../../__tests__/testUtils';
...
describe('countOpenWorkerSlots', () => {
	...
	const create = (
		...
		const card: CompanyCard = {
			...
			fullyAutomated,
			workers: Array.from({ length: size }, (_, i) => ({
				type: 'unskilled',
				roles:
					slots === 'all'
...
describe('basicActions', () => {
	...
	describe('workingClass:strike', async () => {
		...
		test('run', async () => {
			const { company } = game.getCompanyById(game.data.roles.capitalist.data.companies[0].id);
	...
	describe('workingClass:demonstration', async () => {
		test('conditions:noDemonstration', async () => {
			...
			await expect(nextAndTick('workingClass:demonstration')).rejects.toThrow(/noDemonstration/);
		});
		test('conditions:hasUnemployedWorkers', async () => {
			wc.data.workers = [];
			expect(wc.countOpenWorkerSlots()).toBe(9);
			await expect(nextAndTick('workingClass:demonstration')).rejects.toThrow(/hasUnemployedWorkers/);
		});
		test('conditions:hasUnemployedWorkers', async () => {
			wc.data.workers = [
				createTestWorker(),
