...
import { createGetPopulation, createIncreaseProsperity, createNewWorker, createWorker } from './commonMethods';
...
export default class WorkingClassRole extends AbstractRole<typeof RoleEnum.workingClass, WorkingClassData> {
	...
	countOpenWorkerSlots() {
		const middleClassSlots = _.sum(
			Object.values(this.game.data.roles.middleClass.data.companies).map(c => {
				...
				return d.workers.filter(w => w.roles.includes(RoleNameSchema.enum.workingClass)).length;
			}),
		);
