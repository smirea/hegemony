...
- Company deck fixtures are validated against `shared/logic/cards/companyCards.ts` for ids, production, automation/optional-worker production, wages, and workers. Public Companies include 12 physical cards: the 3-diamond University Hospital, Technical University, and National Public Broadcasting are the 3/4-player starting variants; the 2-diamond Public Hospital, Public University, and Regional TV Station are the 2-player starting variants. Regional TV Station visibly produces 3 Influence.
- Cooperative Farm cards are not in `companyCards.ts`; they are parsed from `fixtures/rules/components.md` as 0-cost Food companies that produce 2 Food and require 3 unskilled Working Class workers.
