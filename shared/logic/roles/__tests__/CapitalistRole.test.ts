...
const { initGame, addInput, nextAndTick, addWorkers, eachWorker, assignWorkers } = createGameUtils();
...
describe.skip('setupRound', () => {
	...
	test('sets up market', () => {
		...
		expect(cap.data.companyMarket).toEqual(['c-food', 'c-influence', 'c-market-1', 'c-market-2']);
	});
	test('only redraws empty slots', () => {
		...
		expect(cap.data.companyMarket.length).toBe(4);
		expect(cap.data.companyMarket).toEqual(['c-influence', 'c-market-1', 'c-market-2', 'c-market-3']);
	});
});
