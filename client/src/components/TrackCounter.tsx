...
const TrackCounter: React.FC<TrackCounterProps> = function TrackCounter({ index, tracks, color, format, trackSizes }) {
	const sizes = trackSizes ?? tracks.map(() => 1);

	if (sizes.length !== tracks.length) {
		throw new Error(`trackSizes.length=${sizes.length} must be the same length as tracks.length=${tracks.length}`);
	}

	return (
		<Root>
			{tracks.map((value, i) => (
				<Tick key={i} data-selected={i === index} size={sizes[i]} color={color} data-format={format}>
