...
export interface Action<PlayerInput extends ZodType = ZodNever> {
	...
	validateInput?: (input: PlayerInput extends ZodNever ? never : z.infer<PlayerInput>) => ValidationResult[];
...
export type ActionEventFromAction<K extends string | number | symbol, A extends Action<any>> =
	A extends Action<ZodNever> ? ActionEvent<K> : A extends Action<infer D> ? ActionEvent<K, D> : never;
...
export const RoleNameNoStateSchema = z.enum([RoleEnum.workingClass, RoleEnum.middleClass, RoleEnum.capitalist]);
